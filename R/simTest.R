...
simTest <- function(
  ...
  ##################################################
  ## set working directory to source required fns ##
  ##################################################
  setwd("C:/Cait 2012/Work/Xavier")
  source("./coalescent.sim.R")
  source("./treeWAS Method/tree.sim.R")
  source("./treeWAS Method/treeWAS.R")
  source("C:/Users/caitiecollins/adegenet/R/sequences.R") ## need for DNAbin2genind fn
  ...
  if(missing(gen.size)) gen.size <- 10000
  if(missing(sim.by)) sim.by <- "locus"
  if(missing(theta)) theta <- 1*2
  if(missing(theta_p)) theta_p <- NULL
  if(missing(n.snp.assoc)) n.snp.assoc <- 0
  if(missing(assoc.option)) assoc.option <- "all"
  if(missing(assoc.prob)) assoc.prob <- 90
  if(missing(p.value)) p.value <- 0.05/10000
  ...
  ###########
  if(set.number == 2){
    ## ensure theta_p is NOT null
    if(is.null(theta_p)) theta_p <- 25
    if(!is.null(phen)) phen <- NULL
    ## ensure no associated SNPs
    if(n.snp.assoc != 0) n.snp.assoc <- 0
    if(!is.null(assoc.prob)) assoc.prob <- NULL    
  ...
  if(from.file==TRUE){
    ...
    if(is.null(file.n)) stop("Use argument file.n to specify which files to read in.")
    
    ## get n.reps for main for loop
    n.reps <- length(file.n)
    
    ## set working directory for the set specified
    wd <- paste("C:/Cait 2012/Work/Xavier/Sims/set", set.number, sep="")
    setwd(wd)
  ...
  for(i in 1:n.reps){
    ...
    #     if(from.file == FALSE){
      ...
      dat.wd <- paste("C:/Cait 2012/Work/Xavier/Sims/set", set.number, "/", sep="")
      setwd(dat.wd)
