...
## EXAMPLE ##
# out <- coalescent.sim(n.ind=100, gen.size=10000, 
#                      sim.by="locus", theta=1*2, 
#                      theta_p=15, phen=NULL,
#                      n.snp.assoc=5, assoc.option="all",
#                      biallelic=TRUE, seed=1, plot=TRUE,
#                      heatmap=FALSE, plot2=FALSE)
      ...
      if(.is.odd(n)){
        ...
        phen.loci <- replace(phen.loci, 
                             ...
                             c(unique(phen.loci)[i], 
                               rep(NA, 
                                   (length(which(phen.loci == unique(phen.loci)[i]))-1)
                               )))
      } 
      if(.is.even(n)) phen.loci[which(phen.loci == unique(phen.loci)[i])] <- NA
