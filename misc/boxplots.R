...
## remove Poisson dist sims..

setwd(dirname)

# evalStats.ori.ori <- evalStats
# df <- evalStats
# table(df$n.subs)
# nrow(df)
#
# ## Drop Poisson_1:
# df <- df[!df$n.subs %in% "Poisson_1",]
# levels(df$n.subs)
# df$n.subs <- droplevels(df$n.subs)
# levels(df$n.subs)
...
# ## check foo:
# bar <- removeFirstN(foo, 5)
# bar <- removeLastN(bar, nchar("_performance.Rdata"))
# length(bar)
# which(!c(1:60) %in% as.numeric(bar)) # 55
...
system.time(
  for(i in 1:length(foo)){ # length(foo)
    ...
    dat[[i]] <- vector("list", length=length(test))
    names(dat[[i]]) <- test
    ## separately:
    for(t in 1:3){
      bar <- temp$res[[t]]$pval.0.01.bonf.count.10.x.n.snps$sig.snps
      # bar <- temp$res[[t]]$pval.0.01.bonf.count.1.x.n.snps$sig.snps
      if(!is.vector(bar)){
        dat[[i]][[t]] <- sort(unique(as.vector(unlist(bar$SNP.locus))))
      }
    }
    ## combined:
    if(length(as.vector(unlist(dat[[i]][1:3]))) > 0){
      treeWAS.all[[i]] <- sort(unique(as.vector(unlist(dat[[i]][1:3]))), decreasing=FALSE)
    }else{
      treeWAS.all[[i]] <- NA
    }
    gc()
...
###################################################
## NEW! Get COMBINED results FOR ALL THRESHOLDS: ##
###################################################
## get uniques(sig.snps) for terminal, simultaneous, subsequent results combined
## for ALL thresholds
#
...
# foo
#
# temp <- get(load(paste("./", foo[1], sep="")))
# thresh.levs <- names(temp$res[[1]])
#
# ## store in dat
# test <- c("terminal", "simultaneous", "subsequent")
#
...
#     temp <- get(load(paste("./", foo[i], sep="")))
#
#     dat[[i]] <- vector("list", length=length(thresh.levs))
#     names(dat[[i]]) <- thresh.levs
#
#     treeWAS.all[[i]] <- vector("list", length=length(thresh.levs))
#     names(treeWAS.all[[i]]) <- thresh.levs
#
#     for(n in 1:length(thresh.levs)){
#
#       dat[[i]][[n]] <- vector("list", length=length(test))
#       names(dat[[i]][[n]]) <- test
#
#       ## separately:
#       for(t in 1:3){
...
#         # bar <- temp$res[[t]]$pval.0.01.bonf.count.1.x.n.snps$sig.snps
#         if(!is.vector(bar)){
#           dat[[i]][[n]][[t]] <- sort(unique(as.vector(unlist(bar$SNP.locus))))
#         }
#       } # end (t) for loop
#
#     ## combined:
#     if(length(as.vector(unlist(dat[[i]][[n]][1:3]))) > 0){
#       treeWAS.all[[i]][[n]] <- sort(unique(as.vector(unlist(dat[[i]][[n]][1:3]))), decreasing=FALSE)
#     }else{
#       treeWAS.all[[i]][[n]] <- NA
#     }
#     } # end (n) for loop
...
perf.ori <- perf <- dat
###################
score1.mean.ori.ori <- score1.mean
dat <- score1.mean
...
snps.assoc.ori.ori <- snps.assoc.ori <- snps.assoc <- dat
###################
treeWAS.all.ori.ori <- treeWAS.all
dat <- treeWAS.all
...
treeWAS.all.ori.ori <- treeWAS.all.ori <- treeWAS.all <- dat
###################
SNP.loci.ori.ori <- SNP.loci
dat <- SNP.loci
...
dat.ori <- dat
dat <- dat[ord]
SNP.loci.ori.ori <- SNP.loci.ori <- SNP.loci <- dat
###################
...
for(i in 1:length(treeWAS.all)){
  snps.names <- c(1:10000)

  test.positive <- treeWAS.all[[i]]
...
performance.ori.ori <- performance.ori <- performance

performance.treeWAS.all <- PERFORMANCE.TREEWAS.ALL <- performance
# performance.terminal <- PERFORMANCE.TERMINAL <- performance
# performance.simultaneous <- PERFORMANCE.SIMULTANEOUS <- performance
# performance.subsequent <- PERFORMANCE.SUBSEQUENT <- performance
#
# perf.treeWAS <- performance.treeWAS.all
# perf.term <- performance.terminal
# perf.sim <- performance.simultaneous
# perf.subsq <- performance.subsequent
#
# # n.subs <- c(rep("dist_0", 10), rep("dist_0.01", 10), rep("dist_0.05", 10), rep("dist_0.1", 10))
# # n.subs <- c(n.subs, n.subs)
#
# df <- perf.treeWAS
# df <- cbind(n.subs, df)
# perf.treeWAS <- df
...
# for(i in 1:length(treeWAS.all)){
#
#   performance[[i]] <- vector("list", length=length(thresh.levs))
#   names(performance[[i]]) <- thresh.levs
#
#   for(n in 1:length(treeWAS.all[[i]])){
#     snps.names <- c(1:10000)
#
#     test.positive <- treeWAS.all[[i]][[n]]
#     # test.positive <- SNP.loci[[i]]$terminal
#     # test.positive <- SNP.loci[[i]]$simultaneous
#     # test.positive <- SNP.loci[[i]]$subsequent
#
...
#
#     ## combine eval metrics into df ##
#     performance[[i]][[n]] <- data.frame(accuracy, specificity, FPR, FNR, sensitivity, PPV, FDR, F1.score)
#
#   } # end (n) for loop
#
#   performance[[i]] <- do.call(rbind, performance[[i]])
#
# } # end (i) for loop
...
## NOTE THAT IN treeWAS.all & performance, ORDER OF THRESH.LEVS DOES NOT CORRESPOND TO THRESH.LEVS 1:32.
## NEED TO LABEL BY NUMBERS AFTER SORTED BY CHARACTER!


#################################################################################################
#############################################
## SAVE SNP.loci, treeWAS.all, performance ##
#############################################
# SNP.LOCI <- SNP.loci
# TREEWAS.ALL <- treeWAS.all
# PERFORMANCE <- performance
#
# SNP.loci <- SNP.LOCI[1:40]
# treeWAS.all <- TREEWAS.ALL[1:40]
# performance <- PERFORMANCE[1:40, ]
# save(SNP.loci, file="./set3_rtree_s_10_101_140_treeWAS_SNP.loci.Rdata")
# save(treeWAS.all, file="./set3_rtree_s_10_101_140_treeWAS.all.Rdata")
# save(performance, file="./set3_rtree_s_10_101_140_treeWAS_performance.Rdata")
#
# SNP.loci <- SNP.LOCI[41:80]
# treeWAS.all <- TREEWAS.ALL[41:80]
# performance <- PERFORMANCE[41:80, ]
# save(SNP.loci, file="./set3_rtree_s_50_141_180_treeWAS_SNP.loci.Rdata")
# save(treeWAS.all, file="./set3_rtree_s_50_141_180_treeWAS.all.Rdata")
# save(performance, file="./set3_rtree_s_50_141_180_treeWAS_performance.Rdata")
#
# SNP.loci <- SNP.LOCI
# treeWAS.all <- TREEWAS.ALL
# performance <- PERFORMANCE
...
# save(SNP.loci, file="./set2_ACE_coal_1_54_56_60_treeWAS_SNP.loci.Rdata")
# save(treeWAS.all, file="./set2_ACE_coal_1_54_56_60_treeWAS.all.Rdata")
# save(performance, file="./set2_ACE_coal_1_54_56_60_treeWAS_performance.Rdata")

save(SNP.loci, file="./set2_ACE_coal_1_54_56_60_treeWAS_SNP.loci_ALL_THRESHOLDS.Rdata")
save(treeWAS.all, file="./set2_ACE_coal_1_54_56_60_treeWAS.all_ALL_THRESHOLDS.Rdata")
save(performance, file="./set2_ACE_coal_1_54_56_60_treeWAS_performance_ALL_THRESHOLDS.Rdata")


################################################################################################
####################################
## ADD performance into evalStats ##
####################################
...
info <- df[df$assoc.test=="terminal", c(2:6)]
assoc.test <- rep("treeWAS.all", 59)
temp <- cbind(assoc.test, info, performance)

df <- rbind(temp, df)
head(df)
levels(df$assoc.test)

# EVALSTATS.TREEWAS <- df

levels(df$assoc.test)[c(7, 9, 10, 8, 6, 5, 1:4)]

df$assoc.test <- ordered(df$assoc.test, levels = levels(df$assoc.test)[c(7, 9, 10, 8, 6, 5, 1:4)]) # [c(2:7, 1, 8:10)]
...
evalStats <- df
save(evalStats, file="./set2_ACE_coal_1_54_56_60_treeWAS_ALL_all_tests_best_thresh_evalStats.Rdata")

###########################

##################################################
## ADD PROPER treeWAS.all TO EVALSTATS.THRESH!! ##
##################################################

# EVALSTATS.ORI <- evalStats

evalStats.thresh.old <- evalStats.thresh

df <- evalStats.thresh

## check that df$test is correct!
## need to match levels to sorted as character not 1:32 order!

ord <- match(thresh.levs, sort(as.character(thresh.levs)))
# thresh.levs[ord] ## check
identical(as.character(head(df$test, 32)), as.character(ord)) ## all good.

perf.all <- do.call(rbind, performance)

info <- df[df$assoc.test == "terminal", c(2:8)]
assoc.test <- rep("treeWAS.all", 80*32)
temp <- cbind(assoc.test, info, perf.all)
...
evalStats.thresh <- EVALSTATS.THRESH <- evalStats

###########
## SAVE! ##
###########
save(evalStats, file="./set2_ACE_coal_1_54_56_60_treeWAS_ALL_THRESHOLDS_w_treeWAS_all_evalStats_w_NaN.Rdata")

# save(evalStats, file="./set2_ACE_coal_1_54_56_60_treeWAS_ALL_THRESHOLDS_evalStats.Rdata")


## check for NaN F1.scores... ##
df <- evalStats
...
save(evalStats, file="./set2_ACE_coal_1_54_56_60_treeWAS_ALL_THRESHOLDS_w_treeWAS_all_evalStats.Rdata")


###########################

## (?) REPLACE treeWAS.all w performance w thresh 17 & treeWAS tests w evalStats.thresh.17 (?): ##
head(evalStats)
evalStats.ori.ori <- evalStats
df <- evalStats
df[df$assoc.test=="treeWAS.all", 8:ncol(df)] <- performance

df[df$assoc.test %in% c("terminal", "simultaneous", "subsequent"), ] <- evalStats.thresh.17

EVALSTATS <- evalStats <- df

save(evalStats, file="./set2_ACE_coal_1_54_56_60_ALL_all_tests_best_thresh_evalStats.Rdata")
...
# evalStats <- get(load("./set2_ACE_coal_1_54_56_60_treeWAS_ALL_THRESHOLDS_evalStats.Rdata"))
...
tree.type <- rep("coal", 59*3)
# tree.type <- rep("rtree", 80)
...
# n.subs <- c(n.subs)
# n.subs <- c(rep("dist_0", 10), rep("dist_0.01", 10), rep("dist_0.05", 10), rep("dist_0.1", 10)) #, rep("Poisson_1", 20))
n.subs <- c(rep("dist_0", 20), rep("dist_0.01", 20), rep("dist_0.05", 19)) #, rep("Poisson_1", 20))
n.subs <- c(n.subs, n.subs, n.subs)
...
n.phen.arg <- c(rep(10, 40), rep(5, 40))
var <- n.phen.arg
...
# DF <- data.frame(evalStats[,c("assoc.test")],
#                  "tree.type"=tree.type,
#                  # "s"=s.thresh, "af"=af.thresh,
#                  "n.subs"=n.subs,
#                  "n.phen.arg" = rep(15, 59*3),
#                  "n.phen.subs"=c(n.phen.subs, n.phen.subs, n.phen.subs),
#                  "score1.mean"=c(score1.mean, score1.mean, score1.mean),
#                  evalStats[,c(3:ncol(evalStats))] )
...
## SAVE:
save(evalStats, file="./set2_ACE_coal_1_54_56_60_treeWAS_ALL_THRESHOLDS_evalStats_w_NaN.Rdata") ## WITH NaN values in F1.score & PPV!

# evalStats <- get(load("C:/Cait 2016/Work/Xavier/Sims/set2/set2_ACE_coal_1_54_56_60_treeWAS_ALL_THRESHOLDS_evalStats_w_NaN.Rdata"))
# evalStats <- get(load("C:/Cait 2016/Work/Xavier/Sims/set3/set2_ACE_coal_1_54_56_60_treeWAS_ALL_THRESHOLDS_evalStats_w_NaN.Rdata"))
##########################################################################################################
...
## SAVE:
save(evalStats, file="./set2_ACE_coal_1_54_56_60_treeWAS_ALL_THRESHOLDS_evalStats.Rdata") ## withOUT NaN values!
# save(evalStats, file="C:/Cait 2016/Work/Xavier/Sims/set3/set2_ACE_coal_1_54_56_60_treeWAS_ALL_THRESHOLDS_evalStats.Rdata")
##########################################################################################################
...
for(y in 1:length(Y)){ #
  for(t in 1:3){
    ...
    # df <- df[df$n.phen.arg==5, ]
    # df <- df[df$s==50,]
    beeswarmPlot(y=Y[y], x="test", df, y.lab=NULL,
                 pt.size=4, x.text=TRUE, x.text.size = 17, y.text.size = 17, y.title.size = 26, mean=TRUE)
  }
}
...
set2_ACE_coal_1_54_56_60_BY_THRESH_terminal_F1score
set2_ACE_coal_1_54_56_60_BY_THRESH_simultaneous_F1score
set2_ACE_coal_1_54_56_60_BY_THRESH_subsequent_F1score

set2_ACE_coal_1_54_56_60_BY_THRESH_terminal_PPV
set2_ACE_coal_1_54_56_60_BY_THRESH_simultaneous_PPV
set2_ACE_coal_1_54_56_60_BY_THRESH_subsequent_PPV

set2_ACE_coal_1_54_56_60_BY_THRESH_terminal_sensitivity
set2_ACE_coal_1_54_56_60_BY_THRESH_simultaneous_sensitivity
set2_ACE_coal_1_54_56_60_BY_THRESH_subsequent_sensitivity

set2_ACE_coal_1_54_56_60_BY_THRESH_terminal_FPR
set2_ACE_coal_1_54_56_60_BY_THRESH_simultaneous_FPR
set2_ACE_coal_1_54_56_60_BY_THRESH_subsequent_FPR

## AND SAVE ANOTHER NARROW/SHORT VERSION OF F1SCORE PLOTS:
set2_ACE_coal_1_54_56_60_BY_THRESH_terminal_F1score_short
set2_ACE_coal_1_54_56_60_BY_THRESH_simultaneous_F1score_short
set2_ACE_coal_1_54_56_60_BY_THRESH_subsequent_F1score_short
###############################

## save as pdf w width and height:
## device size: 10.41 x 6.13

###################################     #######################
## PLOT WITH treeWAS.tests POOLED ( --> PROPER treeWAS.all): ##
###################################     #######################
Y <- c("F1.score", "PPV", "sensitivity", "FPR")

for(y in 1:length(Y)){
    df <- evalStats
    df <- df[df$assoc.test=="treeWAS.all", ]
    # df <- df[df$n.phen.arg==5, ]
    # df <- df[df$s==50,]
    beeswarmPlot(y=Y[y], x="test", df, y.lab=NULL,
                 pt.size=4, x.text=TRUE, x.text.size = 17, y.text.size = 17, y.title.size = 26, mean=TRUE)
...
set2_ACE_coal_1_54_56_60_BY_THRESH_treeWAS_all_F1score_proper
set2_ACE_coal_1_54_56_60_BY_THRESH_treeWAS_all_PPV_proper
set2_ACE_coal_1_54_56_60_BY_THRESH_treeWAS_all_sensitivity_proper
set2_ACE_coal_1_54_56_60_BY_THRESH_treeWAS_all_FPR_proper

## AND SAVE ANOTHER NARROW/SHORT VERSION OF F1SCORE PLOTS:
set2_ACE_coal_1_54_56_60_BY_THRESH_treeWAS_all_F1score_short_proper
...
EVALSTATS.ORI <- evalStats
# evalStats <- evalStats[evalStats$s==10,]

# evalStats <- EVALSTATS.ORI
# evalStats <- evalStats[evalStats$s==50,]

## Get ORDER of tests!!!!
df <- evalStats
df <- df[df$assoc.test=="terminal",]
ord <- df[(1):(32),"test"]
ord <- match(ord, sort(ord, decreasing=F))

df <- evalStats
n.levs <- length(levels(df$test))

## by SIM RUN:
F1.ranks.min <- F1.ranks.avg <- F1.ranks.sports <- vector("list", 3)
names(F1.ranks.min) <- names(F1.ranks.avg) <- names(F1.ranks.sports) <- treeWAS.tests
...
for(t in 1:length(treeWAS.tests)){
  ...
  for(i in 1:(nrow(df.complete)/32)){
    ...
    ## rank F1.scores
    ## Min:
    F1.ranks.min[[t]][[i]] <- rank(1 - (df$F1.score), ties="min")[ord]
    ## Avg:
    F1.ranks.avg[[t]][[i]] <- rank(1 - (df$F1.score), ties="average")[ord]
    ## Sports:
    foo <- rank(df$F1.score, ties="min")
    F1.ranks.sports[[t]][[i]] <- sapply(c(1:length(foo)),
                                        function(e) which(sort(unique(foo), decreasing=TRUE) == foo[e]))[ord]
  }
  ## Min:
  F1.ranks.min[[t]] <- do.call(rbind, F1.ranks.min[[t]])
  colnames(F1.ranks.min[[t]]) <- c(1:32)
  ## Avg:
  F1.ranks.avg[[t]] <- do.call(rbind, F1.ranks.avg[[t]])
  colnames(F1.ranks.avg[[t]]) <- c(1:32)
  ## Sports:
  F1.ranks.sports[[t]] <- do.call(rbind, F1.ranks.sports[[t]])
  colnames(F1.ranks.sports[[t]]) <- c(1:32)
} # end for loop
#########
## Store ranks by sim (and treeWAS assoc.test)
## Save BOTH ranks by min and sports (and average?):

save(F1.ranks.min, file="./set2_ACE_coal_1_54_56_60_all_F1.ranks.min.Rdata")
save(F1.ranks.avg, file="./set2_ACE_coal_1_54_56_60_all_F1.ranks.avg.Rdata")
save(F1.ranks.sports, file="./set2_ACE_coal_1_54_56_60_all_F1.ranks.sports.Rdata")

# ########################################################
# ## SANITY CHECK -- COMPARING THRESH 17 and THRESH 18: ##
# ########################################################
#
# ## Set 1: ##
# df <- evalStats
# df <- df[df$assoc.test=="simultaneous",]
#
# summary(df$F1.score ~ df$test)
# foo <- data.frame(df$F1.score[df$test==17], df$F1.score[df$test==18])
#
# length(which(foo[,1] == 0))
# length(which(foo[,2] == 0))
# foo[,1][(which(foo[,2] == 0))]
#
# F1 <- F1.ranks.sports[[1]]
# F1.avg <- F1.ranks.avg[[1]]
# F1.min <- F1.ranks.min[[1]]
#
# bar <- data.frame(F1[,c(17,18)], round(foo, 3), F1.avg[,c(17,18)], F1.min[,c(17,18)])
# names(bar) <- NULL
#
# bar[78,]
#
# 77*32+1+32
#
# ord <- df[(77*32+1):(77*32+32),"test"]
# ord <- match(ord, sort(ord, decreasing=F))
# lala <- data.frame(F1[78,], df[(77*32+1):(77*32+32),"F1.score"][ord], F1.avg[78,], F1.min[78,])
# names(lala) <- NULL
# lala
...
# save(F1.ranks, file="C:/Cait 2016/Work/Xavier/Sims/set3/set2_ACE_coal_1_54_56_60_all_F1.ranks.Rdata")

################################################
## PLOT ranks by sim (and treeWAS assoc.test) ##
################################################
F1.ranks <- F1.ranks.min
F1.ranks <- F1.ranks.avg
F1.ranks <- F1.ranks.sports

for(i in 1:3){
  ymax <- max(F1.ranks[[i]])
  boxplot(F1.ranks[[i]], col=transp(rainbow(32), 0.5), ylim=c(ymax, 0.9), cex.axis=0.7)
  ggplot.bg(grid.nx=32, grid.ny=9)
  boxplot(F1.ranks[[i]], col=transp(rainbow(32), 0.5), ylim=c(ymax, 0.9), cex.axis=0.7, ylab="Rank", add=T)
  points(x=c(1:32), y=colMeans(F1.ranks[[i]], na.rm=TRUE), pch=18, col="red", cex=1.5)
}

## Save w width = 10.41, height = 3.89 inches
## SAVE: ##
set2_ACE_coal_1_54_56_60_ALL_BOXPLOT_compare_thresholds_by_F1score_RANK_min_terminal
set2_ACE_coal_1_54_56_60_ALL_BOXPLOT_compare_thresholds_by_F1score_RANK_min_simultaneous
set2_ACE_coal_1_54_56_60_ALL_BOXPLOT_compare_thresholds_by_F1score_RANK_min_subsequent
## SAVE: ##
set2_ACE_coal_1_54_56_60_ALL_BOXPLOT_compare_thresholds_by_F1score_RANK_avg_terminal
set2_ACE_coal_1_54_56_60_ALL_BOXPLOT_compare_thresholds_by_F1score_RANK_avg_simultaneous
set2_ACE_coal_1_54_56_60_ALL_BOXPLOT_compare_thresholds_by_F1score_RANK_avg_subsequent
## SAVE: ##
set2_ACE_coal_1_54_56_60_ALL_BOXPLOT_compare_thresholds_by_F1score_RANK_sports_terminal
set2_ACE_coal_1_54_56_60_ALL_BOXPLOT_compare_thresholds_by_F1score_RANK_sports_simultaneous
set2_ACE_coal_1_54_56_60_ALL_BOXPLOT_compare_thresholds_by_F1score_RANK_sports_subsequent
#####################################


## COMBINED:
F1.ranks.mat <- do.call(rbind, F1.ranks.min)
F1.ranks.mat <- do.call(rbind, F1.ranks.avg)
F1.ranks.mat <- do.call(rbind, F1.ranks.sports)

ymax <- max(F1.ranks.mat)
boxplot(F1.ranks.mat, col=transp(rainbow(32), 0.5), ylim=c(ymax, 0.9), cex.axis=0.7)
ggplot.bg(grid.nx=32, grid.ny=9)
boxplot(F1.ranks.mat, col=transp(rainbow(32), 0.5), ylim=c(ymax, 0.9), cex.axis=0.7, ylab="Rank", add=T)
points(x=c(1:32), y=colMeans(F1.ranks.mat, na.rm=TRUE), pch=18, col="red", cex=1.5)

## save combined:
set2_ACE_coal_1_54_56_60_ALL_BOXPLOT_compare_thresholds_by_F1score_RANK_min_treeWAS_COMBINED
set2_ACE_coal_1_54_56_60_ALL_BOXPLOT_compare_thresholds_by_F1score_RANK_avg_treeWAS_COMBINED
set2_ACE_coal_1_54_56_60_ALL_BOXPLOT_compare_thresholds_by_F1score_RANK_sports_treeWAS_COMBINED
#####################################
...
# ## HOW TO INTERPRET DIFFERENT RANKING METHODS? ##
# ##############
# ## AVERAGE: ##
# ##############
# rank(1-c(rep(1.0,32)), ties="average")
# # 16.5 16.5 16.5 ... 16.5
# rank(1-c(rep(1.0,31), 0.2), ties="average")
# # 16 16 16 ... 16 32
# rank(1-c(rep(0.2,31), 1.0), ties="average")
# # 17 17 17 ... 17 1
# rank(1-c(1.0, rep(0.3,30), 0.2), ties="average")
# # 1 16.5 16.5 .. 16.5 32
# rank(1-seq(1.0, 0.0, length.out=32), ties="average")
# # 1 2 3 4 5 6 ... 30 31 32
# rank(1-c(0, 0.1, 0.3, 0.8, 0.9, 0.99, 1), ties="average")
# # 7 6 5 4 3 2 1
#
# ##########
# ## MIN: ##
# ##########
# rank(1-c(rep(1.0,32)), ties="min")
# # 1 1 1 1 1 ... 1
# rank(1-c(rep(1.0,31), 0.2), ties="min")
# # 1 1 1 1 1 ... 1 32
# rank(1-c(rep(0.2,31), 1.0), ties="min")
# # 2 2 2 2 2 ... 2 1
# rank(1-c(1.0, rep(0.3,30), 0.2), ties="min")
# # 1 2 2 2 2 2 ... 2 32
# rank(1-seq(1.0, 0.0, length.out=32), ties="min")
# # 1 2 3 4 5 6 ... 30 31 32
# rank(1-c(0, 0.1, 0.3, 0.8, 0.9, 0.99, 1), ties="min")
# # 7 6 5 4 3 2 1
#
# #############
# ## SPORTS: ##
# #############
# foo <- rank(c(rep(1.0,32)), ties="min")
# sapply(c(1:length(foo)),
#        function(e) which(sort(unique(foo), decreasing=TRUE) == foo[e]))
# # 1 1 1 1 1 ... 1
# foo <- rank(c(rep(1.0,31), 0.2), ties="min")
# sapply(c(1:length(foo)),
#        function(e) which(sort(unique(foo), decreasing=TRUE) == foo[e]))
# # 1 1 1 1 1... 1 2
# foo <- rank(c(1.0, rep(0.3,30), 0.2), ties="min")
# sapply(c(1:length(foo)),
#        function(e) which(sort(unique(foo), decreasing=TRUE) == foo[e]))
# # 1 2 2 2 2 2 ... 2 3
# foo <- rank(seq(1.0, 0.0, length.out=32), ties="min")
# sapply(c(1:length(foo)),
#        function(e) which(sort(unique(foo), decreasing=TRUE) == foo[e]))
# # 1 2 3 4 5 6 ... 30 31 32
# foo <- rank(c(0, 0.1, 0.3, 0.8, 0.9, 0.99, 1), ties="min")
# sapply(c(1:length(foo)),
#        function(e) which(sort(unique(foo), decreasing=TRUE) == foo[e]))
# # 7 6 5 4 3 2 1
...
## get info:
info <- evalStats[evalStats$assoc.test=="terminal", 2:6] # 2:7
# df.other <- cbind(rbind(info, info, info, info, info, info, info), df.other)
...
#                                  rep("dapc", nrow(dapc)),
#                                  rep("cmh", nrow(cmh)),
#                                  rep("treeWAS.all", nrow(treeWAS.all))),
#                   df.other)
...
## SAVE:
save(evalStats, file="./set2_ACE_coal_1_54_56_60_ALL_all_tests_best_thresh_evalStats_w_NaN.Rdata")


# evalStats <- get(load("C:/Cait 2016/Work/Xavier/Sims/set2/set2_ACE_coal_1_54_56_60_ALL_all_tests_best_thresh_evalStats.Rdata"))
...
## get ordered levels:
# require(plyr)
levels(df$assoc.test)[c(6, 8, 9, 7, 5, 4, 10, 1, 2, 3)] ## check...
df$assoc.test <- ordered(df$assoc.test, levels = levels(df$assoc.test)[c(6, 8, 9, 7, 5, 4, 10, 1, 2, 3)]) # SAVED!
...
# evalStats.set3.ori <- evalStats.set3
...
## SAVE PLOTS AS PDF ##
#######################
set2_ACE_coal_1_54_56_60_BY_ASSOC_TEST_F1score
set2_ACE_coal_1_54_56_60_BY_ASSOC_TEST_PPV
set2_ACE_coal_1_54_56_60_BY_ASSOC_TEST_sensitivity
set2_ACE_coal_1_54_56_60_BY_ASSOC_TEST_FPR
...
# df <- evalStats
# n.subs <- as.character(df$n.subs)
# n.subs <- replace(n.subs, which(n.subs == "dist_0.5"), "dist_0.05")
# df$n.subs <- n.subs
# evalStats <- df

# N.SUBS <- c("dist_0", "dist_0.1") # "Poisson_1"
...
## SAVE PLOTS AS PDF ##
#######################
set2_ACE_coal_1_54_56_60_BY_ASSOC_TEST_n_subs_dist_0_F1score
set2_ACE_coal_1_54_56_60_BY_ASSOC_TEST_n_subs_dist_0_PPV
set2_ACE_coal_1_54_56_60_BY_ASSOC_TEST_n_subs_dist_0_sensitivity
set2_ACE_coal_1_54_56_60_BY_ASSOC_TEST_n_subs_dist_0_FPR

set2_ACE_coal_1_54_56_60_BY_ASSOC_TEST_n_subs_dist_0_01_F1score
set2_ACE_coal_1_54_56_60_BY_ASSOC_TEST_n_subs_dist_0_01_PPV
set2_ACE_coal_1_54_56_60_BY_ASSOC_TEST_n_subs_dist_0_01_sensitivity
set2_ACE_coal_1_54_56_60_BY_ASSOC_TEST_n_subs_dist_0_01_FPR

set2_ACE_coal_1_54_56_60_BY_ASSOC_TEST_n_subs_dist_0_05_F1score
set2_ACE_coal_1_54_56_60_BY_ASSOC_TEST_n_subs_dist_0_05_PPV
set2_ACE_coal_1_54_56_60_BY_ASSOC_TEST_n_subs_dist_0_05_sensitivity
set2_ACE_coal_1_54_56_60_BY_ASSOC_TEST_n_subs_dist_0_05_FPR

set2_ACE_coal_1_54_56_60_BY_ASSOC_TEST_n_subs_dist_0_1_F1score
set2_ACE_coal_1_54_56_60_BY_ASSOC_TEST_n_subs_dist_0_1_PPV
set2_ACE_coal_1_54_56_60_BY_ASSOC_TEST_n_subs_dist_0_1_sensitivity
set2_ACE_coal_1_54_56_60_BY_ASSOC_TEST_n_subs_dist_0_1_FPR
...
## TROUBLESHOOTING! ##
######################
## treeWAS.all perf >> treeWAS.tests separately?? (set2_rtree_n_phen_5, n.subs==dist_0)
df <- evalStats
df <- df[df$n.phen.arg==5, ]
df <- df[df$n.subs == "dist_0", ]

summary(df$sensitivity ~ df$assoc.test)

snp <- SNP.loci[41:80]
tre <- TREEWAS[41:80]
snp.ass <- snps.assoc[41:80]

sens <- sens.all <- sens.sep <- sens.match <- list()

for(i in 1:40){
  sens.all[[i]] <- length(which(tre[[i]] %in% snp.ass[[i]]))
  sens.sep[[i]] <- vector("list", length=3)
  names(sens.sep[[i]]) <- c("terminal", "simultaneous", "subsequent")
  for(t in 1:3){
    sens.sep[[i]][[t]] <- length(which(snp[[i]][[t]] %in% snp.ass[[i]]))
  }
  sens.match[[i]] <- sens.all[[i]] <= sum(as.vector(unlist(sens.sep[[i]])))
} # end for (i) loop

## CHECK:
all(as.vector(unlist(sens.match)) == TRUE) # TRUE
## get sensitivity by n.subs:
mean((as.vector(unlist(sens.all))/10)[31:40])
0.8
0.4
0.4
0.3
## treeWAS.all looks right. Problem w separate tests???
x <- sens.sep[1:10]

terminal <- simultaneous <- subsequent <- list()
for(i in 1:length(x)) terminal[[i]] <- x[[i]]$terminal
for(i in 1:length(x)) simultaneous[[i]] <- x[[i]]$simultaneous
for(i in 1:length(x)) subsequent[[i]] <- x[[i]]$subsequent
terminal <- as.vector(unlist(terminal))
simultaneous <- as.vector(unlist(simultaneous))
subsequent <- as.vector(unlist(subsequent))

mean(terminal)/10 # 0.2
mean(simultaneous)/10 # 0.8
mean(subsequent)/10 # 0.15
...
#########
## set 3 dist_0 PPV for treeWAS.all << 3 treeWAS tests' PPV separately... How??

# summary(df$PPV ~ df$assoc.test)
# hist(df$PPV[df$assoc.test=="terminal"], breaks=10)
# hist(df$PPV[df$assoc.test=="simultaneous"], breaks=10)
# hist(df$PPV[df$assoc.test=="subsequent"], breaks=10)
# hist(df$PPV[df$assoc.test=="treeWAS.all"], breaks=10)
#
# sm <- summary(df$FPR ~ df$assoc.test)
...
wd <- "E:/treeWAS_Sims/set1/coaltree/parsimony/"
wd.ori <- getwd()
...
for(j in 1:length(file.ns)){
  ...
  SCORE3 <- get(load(paste("./set", set.n, "_", file.n, "_score3.Rdata", sep="")))
  SCORE3.ori <- SCORE3

  snps <- get(load(paste("./set", set.n, "_", file.n, "_snps.Rdata", sep="")))
  phen <- get(load(paste("./set", set.n, "_", file.n, "_phen.Rdata", sep="")))
  snps.assoc <- get(load(paste("./set", set.n, "_", file.n, "_performance.Rdata", sep="")))$snps.assoc

  res <- get(load(paste("./set", set.n, "_", file.n, "_res.Rdata", sep="")))
  ...
  ## get.score3 ##
  ################
  get.score3 <- function(Pa, Pd, Sa, Sd, l=NULL, version="mod"){

    score3 <- NULL

    if(version == "mod"){

      ## Re-centre around 0, from -1 to 1? (NOT WORKING! )
...
#
# filename <- paste("./", set.n, "_", dir.n, "_", tree.type, "_pca.Rdata", sep="")
# save(pca, file=filename)
# filename <- paste("./", set.n, "_", dir.n, "_", tree.type, "_dapc.Rdata", sep="")
# save(dapc, file=filename)
#
# ## summarise
# summary(fisher.bonf)
# summary(fisher.fdr)
...
#
# params <- cbind(thresh.sel, tree.type, s, af)
#
# ################
#
#
...
# save(plink.gc.fdr, file=filename)
# filename <- paste("./", set.n, "_", dir.n, "_", tree.type, "_pca_evalStats.df.Rdata", sep="")
# save(pca, file=filename)
# filename <- paste("./", set.n, "_", dir.n, "_", tree.type, "_dapc_evalStats.df.Rdata", sep="")
# save(dapc, file=filename)
#
#
#
...
# ########################################
#
# ## keep only treeWAS results (x3) for thresh == 1 (ie. "pval.0.01.bonf.count.10.x.n.snps")
# df <- evalStats
# df <- df[df$test==18, ]
# ## remove test columns:
...
# nrow(dat)
#
# dat <- dat[seq(18, nrow(dat), 32),]
# dat <- rbind(dat, pca, dapc)
#
# dat.full <- dat
#
...
#
# df.temp <- df
# df <- rbind(df, dat.full[(nrow(dat.full)-19):nrow(dat.full), ])
#
# df.ORI <- df
#
# df <- dat
...
# ## SAVE: ##
# evalStats <- df
# save(evalStats, file="C:/Cait 2016/Work/Xavier/Sims/set3/set3_1_10_all.tests_evalStats.df.Rdata")
#
#
# evalStats <- dat
#
...
#
# df <- df[df$assoc.test %in% levels(df$assoc.test)[c(2, 5, 7, 4, 1, 11, 9, 10)],]
# df$assoc.test <- ordered(df$assoc.test, levels = levels(df$assoc.test)[c(2, 5, 7, 4, 1, 11, 9, 10)]) # [c(2, 3, 5, 6, 7, 8, 4, 1, 11, 9, 10)] # SAVED!
#
# ## SAVE:
# save(evalStats, file="C:/Cait 2016/Work/Xavier/Sims/set3/set3_1_10_all.tests_bonf.only_evalStats.df.Rdata")
#
# evalStats.ori <- evalStats
# evalStats <- df
#
