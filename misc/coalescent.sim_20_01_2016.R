...
## ie. enable the simulation of non-associated SNPs according to an inputted distribution AND the optional 
## simulation of additional phenotypically-associated SNPs AND allow this all to be optionally guided 
## by a user-inputted phenotype for the terminal nodes. 
...
#   source("C:/Users/Caitlin/treeWAS/pkg/R/utils.R")
  ...
  if(sim.by=="locus"){
    ...
    ##  ## ASSOCIATED SNPS ##  ##
        ...
        if(n.snp.assoc != 0){
          ...
          ## it is currently just running the same code as assoc.option="none".
          
          if(assoc.option=="model"){
  ...
  if(!is.null(snps.assoc)){
    ...
    snps.assoc.new <- list()
    for(i in 1:length(snps.assoc)){
      if(length(which(fixed.loci < snps.assoc[i])) == 0){
        snps.assoc.new[[i]] <- snps.assoc[i]
      }else{
        snps.assoc.new[[i]] <- 
          snps.assoc[i] - length(which(fixed.loci < snps.assoc[i]))
      }    
    } # end for loop
