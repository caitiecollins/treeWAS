...
print.treeWAS.temp <- function(x, sort.by.p = FALSE){
  cat("\t#################### \n")
  cat("\t## treeWAS output ## \n")
  cat("\t#################### \n")
  cat("\t \n")
...
treeWAS.temp <- function(snps,
  ...
  save(snps.sim, file=sprintf('%s.snps_sim.Rdata', prefix))
  ...
  if(any(c("simultaneous", "subsequent") %in% test)){
    ...
    save(snps.rec, file=sprintf('%s.snps_rec.Rdata', prefix))
    ...
    save(snps.sim.rec, file=sprintf('%s.snps_sim_rec.Rdata', prefix))
