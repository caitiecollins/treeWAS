...
For genetic data, the `read.dna` function can read in sequences from "sequential", "CLUSTAL", or "FASTA" formatted files. 
One way to convert the resulting `DNAbin` class object to a binary matrix is by way of the `DNAbin2genind` function.
```{r, eval=FALSE}
## Read data from file
...
To read in phylogenetic trees from file, `read.tree` can be used for trees stored in Newick/New Hampshire format and `read.nexus` for trees of Nexus format. Both return a `phylo` object, so no further conversion is required.
```{r, eval=FALSE}
